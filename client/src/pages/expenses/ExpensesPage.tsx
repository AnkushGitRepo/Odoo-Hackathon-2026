  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-extrabold tracking-tight sm:text-3xl">Fuel & Expenses</h1>
        {hasFullAccess && (
          <div className="flex gap-3">
            <button
              onClick={() => setShowFuelModal(true)}
              className="flex items-center gap-2 rounded-lg border border-mist-200 bg-white px-4 py-2 text-sm font-semibold text-ink-700 shadow-sm hover:bg-mist-50"
            >
      ...
      {!pending && (
        <div className="mt-6 flex items-center justify-between rounded-2xl bg-indigo-50 px-6 py-4 border border-indigo-100">
          <div className="flex items-center gap-3">
            <div className="rounded-full bg-indigo-200 p-2 text-indigo-800">
              <IndianRupee className="size-5" />
            </div>
            <div>
              <p className="text-xs font-bold tracking-wide text-indigo-600 uppercase">Total Operational Cost (Auto) = Fuel + Maintenance</p>
              <p className="text-2xl font-extrabold text-indigo-900">₹{totalOperationalCost.toLocaleString("en-IN")}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-2">
        <div>
          <h2 className="text-lg font-bold text-ink-900 mb-4">Operational Cost by Vehicle</h2>
          <div className="overflow-hidden rounded-2xl bg-white shadow-[0_12px_40px_-16px_rgba(22,50,60,0.15)]">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-mist-100">
                {pending ? (
                  <tr><td colSpan={4} className="px-6 py-8 text-center text-mist-500">Loading...</td></tr>
                ) : summaryRows.length === 0 ? (
                  <tr><td colSpan={4} className="px-6 py-8 text-center text-mist-500">No vehicles found.</td></tr>
                ) : (
                  summaryRows.map((row) => (
                    <tr key={row.vehicleId} className="group hover:bg-mist-50/50">
                      <td className="px-6 py-4 font-medium text-ink-900">
                        {row.name} <span className="text-xs text-ink-500">({row.registrationNumber})</span>
                      </td>
                      <td className="px-6 py-4 text-right text-ink-700">₹{row.tollMiscCost.toLocaleString("en-IN")}</td>
                      <td className="px-6 py-4 text-right text-ink-700">₹{row.maintenanceCost.toLocaleString("en-IN")}</td>
                      <td className="px-6 py-4 text-right font-bold text-ink-900">₹{row.operationalCost.toLocaleString("en-IN")}</td>
                    </tr>
                  ))
                )}
              </tbody>
        ...
        <div>
          <h2 className="text-lg font-bold text-ink-900 mb-4">Recent Fuel Logs</h2>
          <div className="overflow-hidden rounded-2xl bg-white shadow-[0_12px_40px_-16px_rgba(22,50,60,0.15)]">
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-mist-100 text-xs text-ink-500 uppercase tracking-wide">
              ...
              <tbody className="divide-y divide-mist-100">
                {pending ? (
                  ...
                  <tr><td colSpan={4} className="px-6 py-8 text-center text-mist-500">No fuel logs found.</td></tr>
                ) : (
                  fuelLogs.slice(0, 10).map((log) => (
                    <tr key={log._id} className="group hover:bg-mist-50/50">
                      ...
                      <td className="px-6 py-4 text-ink-700">{new Date(log.date).toLocaleDateString()}</td>
                      <td className="px-6 py-4 text-right font-medium text-ink-900">{log.liters} L</td>
                      <td className="px-6 py-4 text-right font-bold text-ink-900">₹{log.cost.toLocaleString("en-IN")}</td>
                    </tr>
                  ))
                )}
              </tbody>
      ...
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4 text-sm">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block font-semibold text-ink-700">Liters</label>
              <input required type="number" min="0.1" step="0.1" className="mt-1 w-full rounded-lg border border-mist-200 px-3 py-2" value={liters} onChange={e => setLiters(e.target.value)} />
            </div>
            <div>
              <label className="block font-semibold text-ink-700">Total Cost (₹)</label>
...
function AddExpenseModal({ onClose, onAdded }: { onClose: () => void; onAdded: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-900/40 backdrop-blur-sm p-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        <h2 className="text-xl font-bold text-ink-900">Add Expense</h2>
        {error && <div className="mt-4 text-sm text-red-600 bg-red-50 p-3 rounded-lg">{error}</div>}
        <form onSubmit={handleSubmit} className="mt-4 space-y-4 text-sm">
          <div>
            <label className="block font-semibold text-ink-700">Vehicle</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block font-semibold text-ink-700">Category</label>
              <select
                required
                className="mt-1 w-full rounded-lg border border-mist-200 px-3 py-2 bg-white"
                value={category}
            ...
            <div>
              <label className="block font-semibold text-ink-700">Amount (₹)</label>
              <input required type="number" min="0" step="1" className="mt-1 w-full rounded-lg border border-mist-200 px-3 py-2" value={amount} onChange={e => setAmount(e.target.value)} />
            </div>
          </div>
          <div>
            <label className="block font-semibold text-ink-700">Date</label>
          ...
          <div>
            <label className="block font-semibold text-ink-700">Note (Optional)</label>
            <input type="text" className="mt-1 w-full rounded-lg border border-mist-200 px-3 py-2" value={note} onChange={e => setNote(e.target.value)} />
          </div>
